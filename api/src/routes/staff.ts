...
app.get(
  ...
  async (c) => {
    ...
    const slots = await getTimeSlotsBySheet(
      c.env.DB,
      sheetId
    );
    ...
    const slotSummaries = await Promise.all(
      slots.map(async (slot) => {
        const queue = await getCallQueueByTimeSlot(
          c.env.DB,
          slot.id
        );
        ...
        const queueCalling =
          ...
          queue?.status === 'recovery_calling';

        const slotStatus = calcTimeSlotStatus(
          slot,
          queueCalling,
          nowDate
        );
        ...
        if (queue) {
          const waitingRow = await c.env.DB
            .prepare(
              `SELECT COUNT(*) as cnt
        ...
        const summary = {
          ...
          remainingGroups: Math.max(
            0,
            slot.capacity_groups - reserved
          ),
    ...
    if (activeSlot?.queue) {
      ...
      // 次の3件（通常キュー）
      // --------------------------------------------------------
      const nextRows = await c.env.DB
        .prepare(
          `SELECT
             e.*,
             r.ticket_code,
             r.group_size
           FROM call_queue_entries e
           JOIN reservations r
             ON r.id = e.reservation_id
           WHERE e.queue_id = ?
             AND e.queue_type = 'normal'
      ...
      nextEntries = nextRows.results.map((r) => ({
      ...
      // 不在一覧
      // --------------------------------------------------------
      const absentRows = await c.env.DB
        .prepare(
          `SELECT
             e.*,
             r.ticket_code
      ...
      absentList = absentRows.results.map((r) => ({
        ticketCode: r.ticket_code,
        entryId: r.id,
      }));
      ...
      // 救済キュー
      // --------------------------------------------------------
      const recoveryRows = await c.env.DB
        .prepare(
          `SELECT
             e.*,
             r.ticket_code
           ...
           JOIN reservations r
             ON r.id = e.reservation_id
           WHERE e.queue_id = ?
             AND e.queue_type = 'recovery'
             AND e.status IN ('waiting', 'calling')
           ORDER BY e.recovery_seq ASC`
        )
        .bind(q.id)
        .all<any>();

      recoveryQueue = recoveryRows.results.map((r) => ({
        ticketCode: r.ticket_code,
        status: r.status,
      }));
...
app.post(
  ...
  async (c) => {
    ...
    const queue = await c.env.DB
      .prepare(
        'SELECT * FROM call_queues WHERE id = ?'
      )
      .bind(entry.queue_id)
      .first<any>();
...
// ============================================================
app.post(
  '/queues/:queueId/mark-absent-and-next',
  requireAuth,
  async (c) => {
...
app.post(
  ...
  async (c) => {
    ...
    const maxRecovery =
      await getMaxRecoverySeq(
        c.env.DB,
        queueId
      );

    const nextRecoverySeq =
      maxRecovery + 1;
...
// ============================================================
app.get(
  '/sheets/:sheetId/reservations/search',
  requireAuth,
  requireSheetPermission('staff'),
  async (c) => {
    ...
    const rows = await c.env.DB
      .prepare(
        `SELECT
         ...
         FROM reservations r
         JOIN time_slots t
           ON t.id = r.time_slot_id
         WHERE r.sheet_id = ?
           AND r.ticket_code LIKE ?
         LIMIT 10`
      )
      .bind(
        sheetId,
...
// ============================================================
app.post(
  '/reservations/:reservationId/emergency-call',
  requireAuth,
  async (c) => {
    const reservationId =
      parseInt(
    ...
    if (
      ...
      return errorResponse(
        ...
        '失効待ち中は通常即時呼出できません。救済キューを使用してください'
      );
    }
...
app.post(
  ...
  async (c) => {
    ...
    // --------------------------------------------------------
    // もし呼出中だった場合、current_entry_idをクリア
    // --------------------------------------------------------
    await c.env.DB
      .prepare(
        `UPDATE call_queues
